...
- 设置本地管理：播放模式、提示音音量、今日暂停、轻松测试模式、恢复默认设置
...
- 提醒弹出前会尝试暂停常见后台媒体来源，例如浏览器视频、音乐、播客、网易云音乐、微信读书和常见播放器
- 提醒视频默认静音，避免本地素材音轨和后台媒体声音混在一起
- 提醒弹出时只播放一次短提示音，并使用设置里的提示音音量，默认 15%
...
## Background media pause behavior

提醒窗口出现前，App 会先尝试暂停常见后台媒体来源。当前覆盖 Safari、Google Chrome、Microsoft Edge、Brave Browser、Arc、Vivaldi、Opera 等支持 AppleScript 标签页脚本的浏览器，并会对 Music、TV、Spotify、Podcasts、QuickTime Player、VLC、IINA、网易云音乐、微信读书等常见播放器发送暂停尝试。

提醒窗口自己的视频默认静音，所以本地视频素材里的音轨不会和背景声音混在一起。App 只会按设置里的提示音音量播放一次短提示音；提醒关闭后不会自动恢复原来的浏览器视频或音乐。

这是个人使用场景下的 best-effort 行为，不是系统级音频控制器。macOS 没有公开 API 可以安全暂停所有第三方音频来源；Firefox 等不开放安全标签页脚本控制的 App，可能仍需要手动暂停。微信读书目前没有公开 `pause` AppleScript 命令，App 会通过辅助功能读取播放器进度，确认音频正在播放后再按它自己的播放/暂停控件；首次使用时 macOS 可能会要求给 Drinking Project 辅助功能权限。

...
程序每次提醒前都会优先扫描 `视频/视频素材/`，并兼容扫描 `视频/` 第一层。后续新增视频后不需要改代码，也不需要重新告诉 Codex。只要文件格式支持，它就会自动进入随机播放池。
...
当前 v2 界面使用 `WKWebView` 加载本地页面：

```text
Test /ReminderWeb/index.html
```

注意：`Test ` 目录名末尾带一个空格。HTML/CSS/JS 只负责视频上的 HUD 视觉、倒计时展示和点击消息发送；最终确认逻辑仍以 Swift 状态为准。如果没有找到可播放视频，界面会提示检查 `视频/视频素材/`。
